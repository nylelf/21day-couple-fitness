...
import { buildActivityHintForProfile } from "./weeklyActivities.js";
import { buildPeriodHintForProfile } from "./periodSchedule.js";
import { normalizeGoalsFromProfile } from "./preferenceUtils.js";
...
function buildFemalePeriodSection(profile, role, challengeStartDate, dayStart, dayEnd) {
  ...
  return `【女生经期数据 — 推算下一次经期（重要逻辑）】
- 用户填写的「上次经期第一天」= 最近一次月经来潮的第 1 天（已发生的历史记录）
- 系统用「上次经期 + 周期」推算**下一次及之后**的经期，不会对用户刚结束的那次经期再安排调整
- 上次经期：${hint.lastPeriodDate}；周期：${hint.cycleLabel}；挑战开始：${challengeStartDate}
- 预测下一次经期从 ${hint.nextPeriodStart} 开始
- 仅当 21 天挑战与某次预测经期的第 1–5 天重叠时，才列入 periodSchedule：${rangeDetail}；全挑战：${fullChallenge}
- 若无重叠，periodSchedule 必须为 []，不要在计划里写经期调整日

经期日训练原则（仅落在重叠日时写入 periodSchedule）：
- 预测经期第1-5天：瑜伽、散步、轻度拉伸为主，避免大重量臀腿冲击
- title 注明「经期调整日」；meals 温热易消化、补铁
- 经期日优先于外部运动日`;
}
...
export function buildPreferenceAnalysisPrompt(profile, role, challengeStartDate, dayStart, dayEnd) {
  ...
  const activitySection = buildActivityHintForProfile(profile, challengeStartDate, dayStart, dayEnd);
...
说明：
- activitySchedule 的 challengeDay 必须与上方「用户勾选」推算结果一致，不得遗漏
- 用户按周一～周日勾选并填写运动名称，系统已按挑战开始日换算为第几天
...
${role === "female" ? "女生 periodSchedule 仅包含「预测的下一次经期」与 21 天重叠的日期；无重叠必须 []，禁止把挑战前几天都标成经期。" : "男生 periodSchedule 固定 []。"}
...
export function formatPreferenceAnalysisForPlanPrompt(analysis, dayStart, dayEnd) {
  ...
  if (analysis.periodSchedule?.length) {
    ...
    lines.push("- 本段无预测经期重叠（21天挑战未碰上下一次经期第1-5天），勿添加经期调整日");
  }
