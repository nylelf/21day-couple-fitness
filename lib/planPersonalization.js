...
import { cleanActivityLabel } from "./activityParsing.js";
import { normalizeGoalsFromProfile } from "./preferenceUtils.js";
import { detectActivityDaysFromProfile, formatWeeklyActivitiesText } from "./weeklyActivities.js";
...
/** Fixed prescription per tier — light guard uses these defaults for rest hints. */
...
- 每周其他运动（按星期勾选）：${otherActivitiesRaw || "无"}
...
export function finalizePlansWithProfile(plans, profile, role, challengeStartDate, dayStart, dayEnd) {
  ...
  const activityDays = detectActivityDaysFromProfile(profile, challengeStartDate, 21).filter(
...
export function buildPersonalizationFingerprint(profile, role) {
  const goals = normalizeGoalsFromProfile(profile).sort().join(",");
  const other =
    formatWeeklyActivitiesText(profile?.weeklyActivities) ||
    String(profile?.otherActivities || "").trim();
  return [
    role,
