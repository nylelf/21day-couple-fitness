...
export function detectActivityDays(otherActivities, challengeStartDate, totalDays = 21) {
  ...
  const weekdayIndices = parseWeekdayIndicesFromText(text);
  if (!weekdayIndices.length) return [];

  const fallbackLabel = cleanActivityLabel(text);
  ...
  for (let day = 1; day <= totalDays; day += 1) {
    ...
    if (!weekdayIndices.includes(monBased) || seen.has(day)) continue;

    const char = INDEX_TO_CHAR[monBased];
    const label = extractLabelNearWeekday(text, char) || fallbackLabel;
    days.push({ day, weekdayIndex: monBased, label });
    seen.add(day);
  }
...
export function buildPersonalizationPromptBlock(profile, role, challengeStartDate, dayStart, dayEnd) {
  ...
  for (let day = dayStart; day <= dayEnd; day += 1) {
    ...
    if (activity) {
      scheduleLines.push(
        `第${day}天：【其他运动日·强制】用户当日有「${activity.label}」→ 只能3个拉伸动作，title 含「${activity.label}」和「运动日恢复」，禁止芭蕾基训/篮球训练/力量动作（女生尤其禁止把芭蕾课当成当日力量训练）`
      );
    } else {
      scheduleLines.push(
        `第${day}天：${splitLabel}（${rx.tierTag}：每动作${rx.sets}组×${rx.reps}，休息${rx.rest}）`
      );
    }
  }
  ...
  const activityList =
    activityDays.length > 0
      ? activityDays.map((item) => `第${item.day}天=${item.label}`).join("、")
      : "无";
...
- 训练等级：${LEVEL_LABELS[level] || level} → ${rx.promptLine}
- 已识别其他运动日：${activityList}
- 训练目标：${goalText}
...
export function shapePlansWithProfile(plans, profile, role, challengeStartDate, dayStart, dayEnd) {
  if (!plans) return plans;
  const normalized = profile || {};
  const activityDays = detectActivityDays(normalized.otherActivities, challengeStartDate);

  let next = { ...plans };
  for (let day = dayStart; day <= dayEnd; day += 1) {
    const key = `day-${day}`;
    if (!next[key]) continue;
    next[key] = applyProfileToPlan(next[key], normalized, role);
  }
...
export function applyProfileToPlan(plan, profile, role) {
  ...
  if (/运动日恢复|仅拉伸/i.test(plan.title || "")) return plan;
  ...
  workouts = workouts.slice(0, maxExercises).map((item) => {
    const isStrengthMain =
      goals.includes("strength") && /蹲|推|拉|硬拉|squat|press|deadlift|bench/i.test(item.name);
    const noteBase = [rx.noteSuffix, item.note || ""].filter(Boolean).join(" ");
    return {
      ...item,
      sets: rx.sets,
      rest: level === "advanced" && isStrengthMain ? "90-120s" : rx.rest,
      reps: isStrengthMain ? "4-6" : goals.includes("fat_loss") ? "12-15" : rx.reps,
      note: noteBase,
    };
  });
  ...
  let title = String(plan.title || "");
  if (!title.includes(rx.tierTag)) {
    title = `【${rx.tierTag}】${title}`;
  }

  return {
    ...plan,
    title: `${title}${suffix}${equipTag}`,
    workouts,
    meals: plan.meals,
    habits: plan.habits,
  };
